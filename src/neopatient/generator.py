...
import jinja2
import chromadb
from .matcher import find_best_matching_code, batch_find_best_matching_codes
...
def _handle_generation_stage(client: openai.OpenAI, state: Dict[str, Any]) -> Union[List[List[Dict]], Dict[str, Any]]:
...
def _handle_check_generation_stage(client: openai.OpenAI, state: Dict[str, Any]) -> Union[List[List[Dict]], Dict[str, Any]]:
    ...
    try:
        ...
        if batch_status.status == "completed":
            ...
            state["generated_records"] = _parse_generation_results(results, state["cohort_specs"])
            ...
            raise RuntimeError(f"Batch generation failed with status: {batch_status.status}")
...
def _handle_matching_stage(client: openai.OpenAI, state: Dict[str, Any]) -> Union[List[List[Dict]], Dict[str, Any]]:
...
def _start_verification_stage(client: openai.OpenAI, state: Dict[str, Any]) -> Union[List[List[Dict]], Dict[str, Any]]:
...
def _handle_check_verification_stage(client: openai.OpenAI, state: Dict[str, Any]) -> Union[List[List[Dict]], Dict[str, Any]]:
    ...
    try:
        ...
        if batch_status.status == "completed":
            ...
            state["verified_records"] = _parse_verification_results(results, state["code_matched_records"])
            ...
            raise RuntimeError(f"Batch verification failed with status: {batch_status.status}")
...
def _create_jsonl_file(requests: List[Dict]) -> str:
    ...
    try:
        with open(temp_path, 'rb') as f:
            file_response = openai.OpenAI().files.create(
                file=f,
                purpose="batch"
            )
...
def _parse_generation_results(results: List[Dict], cohort_specs: List[Dict]) -> List[List[Dict]]:
    ...
    for result in results:
        if result.get("response", {}).get("status_code") == 200:
            ...
            record_data = json.loads(result["response"]["body"]["choices"][0]["message"]["content"])
...
def _parse_verification_results(results: List[Dict], generated_records: List[List[Dict]]) -> List[List[Dict]]:
    ...
    for result in results:
        if result.get("response", {}).get("status_code") == 200:
            ...
            verification = json.loads(result["response"]["body"]["choices"][0]["message"]["content"])
            cohort_verifications[cohort_idx].append(verification)
    
    return cohort_verifications


def _match_codes(all_cohort_records: List[List[Dict]], chroma_client: chromadb.PersistentClient) -> List[List[Dict]]:
    ...
    for cohort_idx, cohort_records in enumerate(all_cohort_records):
        for record_idx, record in enumerate(cohort_records):
            ...
            for meas_idx, meas in enumerate(record["static_measurements"]):
                ...
                measurement_locations.append((cohort_idx, record_idx, "static", meas_idx))
            ...
            for event_idx, event in enumerate(record["events"]):
                for meas_idx, meas in enumerate(event["measurements"]):
                    all_measurements.append((meas["code_system"].value, meas["code_desc"]))
                    measurement_locations.append((cohort_idx, record_idx, "event", event_idx, meas_idx))
    
    # Perform batch matching if we have measurements to process
    if all_measurements:
        batch_results = batch_find_best_matching_codes(all_measurements, chroma_client)
        ...
        for (code, proper_desc), location in zip(batch_results, measurement_locations):
            if code:  # Only update if we found a match
                ...
                if location[2] == "static":
                    ...
                    record["static_measurements"][meas_idx]["code_desc"] = proper_desc[:128]
                elif location[2] == "event":
                    _, _, _, event_idx, meas_idx = location
                    record["events"][event_idx]["measurements"][meas_idx]["code"] = code
                    record["events"][event_idx]["measurements"][meas_idx]["code_desc"] = proper_desc[:128]
