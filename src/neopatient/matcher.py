...
def find_best_matching_code(coding_system: str, description: str, chroma_client: chromadb.PersistentClient) -> tuple[str, str]:
    """
...
def find_best_matching_codes(
    ...
    for i in range(len(descriptions)):
        if results['metadatas'] and results['metadatas'][i] and len(results['metadatas'][i]) > 0:
...
def batch_find_best_matching_codes(
    ...
    for system, system_queries in system_groups.items():
        ...
        batch_results = find_best_matching_codes(system, list(descriptions), chroma_client)
