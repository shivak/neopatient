...
def setup_databases(parquet_path: str, chroma_db_path: str = "clinprime_chroma"):
    ...
    con = duckdb.connect()
    coding_systems = con.query("SELECT DISTINCT code_system FROM read_parquet($parquet_path)", 
                             params={'parquet_path': parquet_path}).fetchall()
    ...
    for system in coding_systems:
        ...
        sub_data = con.query(
            f"SELECT med_code, t.desc FROM read_parquet($parquet_path) AS t WHERE code_system = $system",
        ...
        metadatas = [
            {
                "med_code": str(sub_data.loc[i, 'med_code']),
                "desc": descs[i]
            } for i in range(len(sub_data))
        ]
        
        # Add documents to collection in batches
        batches = create_batches(api=client, ids=ids, documents=descs, embeddings=embeddings.tolist(), metadatas=metadatas)
...
def load_chroma_client(chroma_db_path: str = "clinprime_chroma") -> chromadb.PersistentClient:
    """
    ...
    if not os.path.exists(chroma_db_path):
        raise FileNotFoundError(f"ChromaDB database not found at path: {chroma_db_path}")
    settings = Settings(anonymized_telemetry=False)
