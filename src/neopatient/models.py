...
class EventMeasurement(BaseModel):
    ...
    code_system: CodeSystem = Field(description="The vocabulary system for this code")
    code_desc: str = Field(description="Brief textual description of the code/measurement/event")
    result_num: Optional[float] = Field(None, description="Numeric result associated with this measurement (e.g., laboratory test result)")
    unit: Optional[str] = Field(None, description="Units of the numerical measurement (e.g. mg/dL, /HPF, cm H2O, mU/L, pmol/L, Sec., mm Hg, lbs)")
...
class MedicalEvent(BaseModel):
    ...
    measurements: List[EventMeasurement] = Field(description="List of all codes recorded during this event")


class PatientRecord(BaseModel):
    """A synthetic longitudinal patient record."""
    patient_id: int = Field(description="Unique patient identifier")
    static_measurements: List[StaticMeasurement] = Field(description="Static measurements for gender, race, and ethnicity")
    events: List[MedicalEvent] = Field(description="List of medical events with their associated measurements")
